...
#define B_BLIZZARD_HAIL             GEN_8 // In Gen4+, Blizzard bypasses accuracy checks if it's hailing.
#define B_SHEER_COLD_ACC            GEN_8 // In Gen7+, Sheer Cold's base chance of hitting is reduced to 20% if the user isn't Ice-typed.
...
// Catching settings
#define B_SEMI_INVULNERABLE_CATCH   GEN_8 // In Gen4+, you cannot throw a ball against a Pokemon that is in a semi-invulnerable state (dig/fly/etc)
...
// Daycare settings
#define INHERIT_BALL				GEN_8 // Starting from gen6, pokemon could inherit pokeballs from their parents
#define INHERIT_IVS 				GEN_8 // Starting from gen6, pokemon could inherit 5 IVs from their parents using Destiny Knot
#define INHERIT_NATURE 				GEN_8 // Starting from gen6, pokemon could inherit their parent's nature regardless of gender using everstone
