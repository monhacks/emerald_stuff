...
static bool8 LoadBagMenu_Graphics(void)
{
    ...
    case 3:
        if (!IsWallysBag() && gSaveBlock2Ptr->playerGender != MALE)
            LoadCompressedPalette(gBagScreenMale_Pal, BG_PLTT_ID(0), 2 * PLTT_SIZE_4BPP); // gBagScreenFemale_Pal
        else
...
static void GetItemName(u8 *dest, u16 itemId)
{
    ...
    case TMHM_POCKET:
        ...
        {
            // Get HM number
            ConvertIntToDecimalStringN(gStringVar1, itemId - ITEM_HM01 + 1, STR_CONV_MODE_LEADING_ZEROS, 3);
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        default:
            ...
            else if (JOY_NEW(START_BUTTON))
            {
                ...
                {
                    PlaySE(SE_FAILURE);
                    //DisplayItemMessage(taskId, FONT_NORMAL, gText_NothingChanged, HandleErrorMessage);
                    break;
                }
