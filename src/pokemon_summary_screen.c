...
const u8 sText_TitleSkills[] = _("Stats");
...
const u8 sText_TitlePageIVs[] = _("{DPAD_LEFTRIGHT}Page {A_BUTTON}Spend EVs");
...
static void PrintNotEggInfo(void)
{
    ...
    //47cea526ec00fb83fd6ae7c8c442a307dd0b099d
...
static void BufferCharacteristicString(void)
{
    ...
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(7, gNatureFlavorPointers[sum->mintNature]);
}
...
static void PrintSkillsPage(void)
{
	...
	for(i = 0; i < NUM_STAT_TYPES; i++){
		switch(i){
			case 0:
				PrintSmallTextOnWindow(PSS_LABEL_PANE_RIGHT, gText_Stats, 67, y, 0, PSS_COLOR_WHITE_BLACK_SHADOW);
			break;
			case 1:
				PrintSmallTextOnWindow(PSS_LABEL_PANE_RIGHT, gText_IVs, 102, y, 0, 1);
			break;
			case 2:
				x = 16;
				PrintSmallTextOnWindow(PSS_LABEL_PANE_RIGHT, gText_EVs, (x * 8)-1, y, 0, PSS_COLOR_WHITE_BLACK_SHADOW);
			break;
		}	
	}
	...
	for(i = 0; i < NUM_STAT_TYPES; i++){
		switch(i){
			case 0:
				...
				StringAppend(gStringVar1, gStringVar2);
				x = 8;
                offset = GetStringCenterAlignXOffset(7, gStringVar1, 37);
		        PrintNarrowTextOnWindow(PSS_LABEL_PANE_RIGHT, gStringVar1, ((x - 1)* 8) + offset, y, 0, PSS_COLOR_WHITE_BLACK_SHADOW);
			break;
			case 1:
				ConvertIntToDecimalStringN(gStringVar1, summary->hpIV, STR_CONV_MODE_LEFT_ALIGN, 3);
				x = 13;  
			...
			case 2:
				...
				x = 16;
                offset = GetStringCenterAlignXOffset(7, gStringVar1, 24);
		        PrintNarrowTextOnWindow(PSS_LABEL_PANE_RIGHT, gStringVar1, ((x - 1)* 8)+1 + offset, y, 0, PSS_COLOR_WHITE_BLACK_SHADOW);
			break;
		}	
        offset = GetStringCenterAlignXOffset(7, gStringVar1, 24);
	}	
	...
	for(i = 0; i < NUM_STAT_TYPES; i++){
		switch(i){
			case 0:
				...
				x = 10;
                offset = GetStringCenterAlignXOffset(7, gStringVar1, 24);
		        PrintNarrowTextOnWindow(PSS_LABEL_PANE_RIGHT, gStringVar1, 70 + offset, y, 0, 1);
			break;
			case 1:
				ConvertIntToDecimalStringN(gStringVar1, summary->atkIV, STR_CONV_MODE_LEFT_ALIGN, 3);
				x = 13;
                offset = GetStringCenterAlignXOffset(7, gStringVar1, 24);
		        PrintNarrowTextOnWindow(PSS_LABEL_PANE_RIGHT, gStringVar1, ((x - 1)* 8)+1 + offset, y, 0, 1);
			break;
			case 2:
...
static void UNUSED CreateHealthBarSprites(u16 tileTag, u16 palTag)
{
    ...
    for (i = 0; i < HP_BAR_SPRITES_COUNT; i++)
    {
        ...
        sHealthBar->spritePositions[i] = i * 8 + 87;
        spriteId = CreateSprite(&template, sHealthBar->spritePositions[i], 27, 0);
