...
static const u8 sText_TopBar_Main_Right[]   = _("{R_BUTTON}Next Page");
static const u8 sText_TopBar_Custom[]       = _("Page 2");
static const u8 sText_TopBar_Custom_Left[]  = _("{L_BUTTON}Prev Page");
...
static void DrawChoices_LevelScaling(int selection, int y)
{
bool8 active = CheckConditions(MENUITEM_CUSTOM_LEVEL_SCALING);
    u8 styles[2] = {0};
    styles[selection] = 1;

    if (selection == 0)
    {
        gSaveBlock2Ptr->optionsLevelScaling = 0;
    }
    else
    {
        gSaveBlock2Ptr->optionsLevelScaling = 1;
    }
